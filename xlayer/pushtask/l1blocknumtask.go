...
import (
	...
	xl_utils "github.com/0xPolygonHermez/zkevm-bridge-service/xlayer/utils"
...
func (t *L1BlockNumTask) doTask(ctx context.Context) {
	...
	oldBlockNum -= min(xl_utils.L1TargetBlockConfirmations.Get(), oldBlockNum)
	blockNum -= min(xl_utils.L1TargetBlockConfirmations.Get(), blockNum)
	if blockNum <= oldBlockNum {
		return
	}
		...
		for _, deposit := range deposits {
			go func(deposit *etherman.Deposit) {
				...
				err := t.messagePushProducer.PushTransactionUpdate(&pb.Transaction{
					...
					GlobalIndex: etherman.GenerateGlobalIndex(true, uint32(t.rollupID-1), deposit.DepositCount).String(),
				})
				if err != nil {
